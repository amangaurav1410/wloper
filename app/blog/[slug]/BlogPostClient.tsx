...
import {
    ArrowLeft, Calendar,
    Twitter, Linkedin, Copy, CheckCircle2,
    Sparkles, Zap, Bot
} from 'lucide-react';
...
export default function BlogPostClient({ slug }: BlogPostClientProps) {
    ...
    const handleCopy = () => {
        navigator.clipboard.writeText(`https://wloper.com/blog/${post.slug}`);
        setCopied(true);
        setTimeout(() => setCopied(false), 2000);
    };

    const handleTwitterShare = () => {
        window.open(`https://twitter.com/intent/tweet?text=${encodeURIComponent(post.title)}&url=${encodeURIComponent(`https://wloper.com/blog/${post.slug}`)}`, '_blank');
    };

    const handleLinkedinShare = () => {
        window.open(`https://www.linkedin.com/shareArticle?mini=true&url=${encodeURIComponent(`https://wloper.com/blog/${post.slug}`)}&title=${encodeURIComponent(post.title)}`, '_blank');
    };
                                ...
                                >
                                    ...
                                    <div className="flex items-center gap-4">
                                        <button
                                            ...
                                            className="w-10 h-10 rounded-full bg-white/5 border border-white/10 flex items-center justify-center hover:bg-wl-accent hover:text-black transition-all"
                                        >
                                        ...
                                        <button
                                            ...
                                            className="w-10 h-10 rounded-full bg-white/5 border border-white/10 flex items-center justify-center hover:bg-wl-accent hover:text-black transition-all"
                                        >
                                            {copied ? <CheckCircle2 className="w-4 h-4 text-wl-accent" /> : <Copy className="w-4 h-4" />}
                                        </button>
                                    </div>
                                </motion.div>
                            </div>
