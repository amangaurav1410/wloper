...
import { X, Send, ShieldCheck, Cpu, Loader2 } from 'lucide-react';
import { useState } from 'react';

interface HireAIModalProps {
    isOpen: boolean;
    onClose: () => void;
    selectedRole: string;
}

export default function HireAIModal({ isOpen, onClose, selectedRole }: HireAIModalProps) {
        ...
        try {
            const response = await fetch('/api/contact', {
                ...
                body: JSON.stringify({
                    ...
                    subject: `Hire AI Request: ${selectedRole}`,
                    message: `Requested to hire: ${selectedRole}\n\nRequirements:\n${formData.requirements}`
                }),
            });
                            ...
                            {!isSuccess ? (
                                <div className="space-y-8">
                                    <div className="space-y-4">
                                        ...
                                        <h2 className="text-3xl md:text-4xl font-black leading-tight">
                                            Hire <span className="text-wl-accent">{selectedRole}</span>
                                        </h2>
                                        <p className="subtitle-md">
                                            Fill out the form below and our deployment team will get back to you immediately to set up your AI employee.
                                        </p>
                                    </div>
                                    ...
                                    <form onSubmit={handleSubmit} className="space-y-6">
                                        ...
                                        <div className="space-y-2">
                                            <label className="text-[10px] font-black uppercase tracking-widest text-wl-muted-dark ml-1">Specific Requirements / Context</label>
                                            <textarea
                                                required
                                                rows={3}
                                                value={formData.requirements}
                                                onChange={(e) => setFormData({ ...formData, requirements: e.target.value })}
                                                className="w-full bg-white/5 border border-white/10 rounded-2xl px-6 py-4 text-white focus:border-wl-accent focus:bg-white/10 focus:outline-none transition-all font-medium resize-none"
                                                placeholder="Tell us a bit about what tasks you want the AI to handle..."
                                            />
                                        </div>
                                        ...
                                        >
                                            {isSubmitting ? (
                                                ...
                                                <span className="flex items-center gap-2">
                                                    Complete Hiring Request <Send className="w-4 h-4 group-hover:translate-x-1 group-hover:-translate-y-1 transition-transform" />
                                                </span>
                                            )}
                                        </button>
                                ...
                                >
                                    ...
                                    <p className="subtitle-md mx-auto mb-10">
                                        Our deployment engineers are reviewing your request. We'll be in touch shortly to initialize your {selectedRole}.
                                    </p>
                                    <button
                                        onClick={onClose}
